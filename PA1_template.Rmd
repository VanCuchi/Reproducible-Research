...
```{r}
data_na<-read.csv2("C:/Users/tt_kb/Documents/Trabajos/DS_Coursera/Reproducible Research/Reproducible-Research/activity.csv", sep = ",")
...
medi
```
The Mean number of steps taken each day is `mean`. 
The Median number of steps taken each day is `medi`.
...
maxmean_interval
```
The 5-minute interval that, on average, contains the maximum number of steps is `maxmean_interval`. 
